...
func d3() {
	...
	numberSoFar := ""    //build up a number we are parsing
	total := 0           //* Count up all numbers seen

	// * Rets 1 if symbol. 0 if number. -1 if .
	// * direction controls what direction to go, -1 and 1 are accepted values
	checkForSymbol := func(i, distance, direction int) int {
		if direction != -1 && direction != 1 {
			panic("direction value is not allowed")
		}

		symbolSet := map[rune]bool{'*': true, '@': true, '!': true, '+': true, '&': true, '$': true, '#': true, '%': true, '^': true, '/': true, '=': true, '-': true}

		if direction == 1 {
			if i+distance < len(inputD3) {
				r := runes[i+distance]
				if symbolSet[r] {
					return 1
			...
			return -1 //failed bounds check pretend its .
		} else {
			if i-distance >= 0 {
				r := runes[i-distance]
				if symbolSet[r] {
	...
	//* cleanup after a number process happens
	resetStateAfterJob := func() {
		symbolPresent = false
		foundNumber = false
		numberSoFar = ""
	}

	//* Remaining symbol checks to do, when you've confirmed that the previous index has been checked already
	doChecksWhenAlreadySawNumber := func(i int) {
		//check top right
		if checkForSymbol(i, 140, -1) == 1 {
			symbolPresent = true
		}

		//check bot right
		if checkForSymbol(i, 142, 1) == 1 {
			symbolPresent = true
		}

		//special case - check right - finding one means we can end further checks right away
		if checkForSymbol(i, 1, 1) == 1 {
			fmt.Println("Ran out of chain due to symbol right, had:", numberSoFar)
			res, _ := strconv.Atoi(numberSoFar)
			total += res
			resetStateAfterJob()
		}
	}

	for i, r := range inputD3 {
		//* we're still processing a found number event from a previous iteration, need to check if number still there first and then if there was a symbol we add it to total
		if foundNumber {
			if unicode.IsDigit(r) {
				numberSoFar += string(r)
				if symbolPresent {
					continue
				}

				//* ATP no symbol yet, but we found another number. Do minimum checks required for this symbol
				doChecksWhenAlreadySawNumber(i)
			} else if symbolPresent { //* Ran out of numbers
				fmt.Println("Ran out of numbers in a chain, had:", numberSoFar)
				res, _ := strconv.Atoi(numberSoFar)
				total += res
				resetStateAfterJob()
			} else {
		...
		} else if !foundNumber && unicode.IsDigit(r) {
			foundNumber = true
			numberSoFar = string(r)
			symbolPresent = false
			//check top left
			if checkForSymbol(i, 142, -1) == 1 {
				// temp := numberSoFar + string(runes[i+1]) + string(runes[i+2])
				// fmt.Println("Found a top left, this and next 2 digits are:", temp)
				symbolPresent = true
				continue
			}
			//check top
			if checkForSymbol(i, 141, -1) == 1 {
				// temp := numberSoFar + string(runes[i+1]) + string(runes[i+2])
				// fmt.Println("Found a top, this and next 2 digits are:", temp)
				symbolPresent = true
				continue
			}
			//check top right
			...
			//check left, since we just found a number
			if checkForSymbol(i, 1, -1) == 1 {
				symbolPresent = true
				continue
			}
			...
			if checkForSymbol(i, 140, 1) == 1 {
				// temp := numberSoFar + string(runes[i+1]) + string(runes[i+2])
				// fmt.Println("Found a bot left, this and next 2 digits are:", temp)
				symbolPresent = true
				continue
			}
			//check bot
			...
			//* There are no single digit numbers so no need to check right for a symbol that could end a single number job
		}
	}
...
const inputD3 = `.854...........................................................................362...........271...732........838.........24................
...*.............................117*...........459........767*648....#.........*...................................$...&..=................
