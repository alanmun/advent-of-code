...
func d2() {
	mapping := map[string]int{"red": 12, "green": 13, "blue": 14}
	sumID := 0

	for _, line := range strings.Split(inputD2, "\n") {
		gameAndData := strings.Split(line, ":")
		fmt.Println(gameAndData)
		gameNum, _ := strconv.Atoi(strings.Replace(gameAndData[0], "Game ", "", -1))
		sets := strings.Split(gameAndData[1], ";")

		viableGame := true
	setLoop:
		for i, set := range sets {
			fmt.Printf("Looking at set %d for game %d\n", i+1, gameNum)
			colorAndCount := strings.Split(set, ",")

			for _, pairing := range colorAndCount {
				parts := strings.Split(pairing, " ") //* There's an extra " " at beginning so we just treat as 1-indexed
				count, _ := strconv.Atoi(parts[1])
				color := parts[2]

				if mapping[color] < count {
					viableGame = false
					fmt.Println("NOT VIABLE:", color, mapping[color], "was:", count)
					break setLoop
				}
			}
		}
...
const inputD2 = `Game 1: 8 green, 4 red, 4 blue; 1 green, 6 red, 4 blue; 7 red, 4 green, 1 blue; 2 blue, 8 red, 8 green
