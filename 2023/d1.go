...
// func main() {
// 	total := 0
// 	for _, line := range strings.Split(inputD1, "\n") {
...
func getTwoDigitNumber(line string) int {
	NUMBER_SET := map[string]bool{"0": true, "1": true, "2": true, "3": true, "4": true, "5": true, "6": true, "7": true, "8": true, "9": true}
	lfound := false
	lp := 0
	rfound := false
	rp := len(line) - 1
	...
	for {
		if lfound && rfound {
			numStr := fmt.Sprintf("%c%c", line[lp], line[rp])
			num, _ := strconv.Atoi(numStr)
			return num
		}

		if lfound || NUMBER_SET[string(line[lp])] {
			// fmt.Println("Found left:", line[lp])
			lfound = true
		} else {
			lp++
		}

		if rfound || NUMBER_SET[string(line[rp])] {
			// fmt.Println("Found right:", line[rp])
			rfound = true
		} else {
			rp--
		}
